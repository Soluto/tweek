...
jest.unmock('../../modules/utils/mutator');

import Mutator from '../../modules/utils/mutator';
...
describe('mutator tests', () => {
  ...
  describe('traversal', () => {
    it('in should allow navigating in object', newMutator((target, mutator) => {
    ...
    it('up should allow navigating in object', newMutator((target, mutator) => {
      target.prop1 = { prop2: 10 };
      expect(mutator.in('prop1').in('prop2').up().getValue().prop2).toEqual(10);
    }));

    it('in & up shoul work with arrays', newMutator((target, mutator) => {
  ...
  describe('insert', () => {
    it('should add a field to object', newMutator((target, mutator) => {
      mutator.insert('myField', 10);
      expect(target.myField).toEqual(10);
    }));

    it('should be able to add a nested field', newMutator((target, mutator) => {
      mutator.insert('someField', {})
             .in('someField')
             .insert('nested', 10);
      expect(target.someField.nested).toEqual(10);
    }));

    it('should be able to add item to an array at index', newMutator((target, mutator) => {
      mutator.insert('someFieldArray', [])
             .in('someFieldArray')
             .insert(0, 5)
             .insert(1, 10);
  ...
  describe('delete', () => {
    it('should be able to delete an item', newMutator((target, mutator) => {
      target.someKey = 5;
      mutator.in('someKey').delete();
      expect(target.someKey).toEqual(undefined);
    }));

    it('should be able to delete nested item', newMutator((target, mutator) => {
      target.someKey = { a: 10 };
  ...
  describe('update value', () => {
    it('should be able to update a field value', newMutator((target, mutator) => {
      target.someKey = 5;
      mutator.in('someKey').updateValue(10);
      expect(target.someKey).toEqual(10);
    }));

    it('should be able to update a value in array', newMutator((target, mutator) => {
  ...
  describe('update key', () => {
    it('should be able to update a key', newMutator((target, mutator) => {
      target.someKey = 5;
      mutator.in('someKey').updateKey('newKey');
      expect(target.someKey).toEqual(undefined);
      expect(target.newKey).toEqual(5);
    }));
    it('should update the mutator path after key is updated', newMutator((target, mutator) => {
      target.someKey = 5;
      mutator.in('someKey').updateKey('newKey')
                           .updateValue(10);
      expect(target.newKey).toEqual(10);
    }));
    it('should preserve order in object', newMutator((target, mutator) => {
  ...
  describe('prepend', () => {
    it('should be able to insert item to the start of an array', newMutator((target, mutator) => {
  ...
  describe('stateless mutator', () => {
    ...
    it('apply should call method onMutation event only once per transcation', () => {
      ...
      mutator.apply(m => m.in('a').updateValue(2)
                                .up()
                                .in('b').updateValue(2));
