...
import { allOperators, getPropertySupportedOperators, equal, inOp } from '../../../../../../../../../services/operators-provider';
...
export default withState('currentInputValue', 'setCurrentInputValue', '')(
  ({mutate, property, suggestedValues, autofocus, currentInputValue, setCurrentInputValue}) => {
    const selectProperty = newProperty => {
      ...
      const newValue = newOperator.getValue(newProperty.defaultValue || '', propertyTypeDetailsToComparer(propertyTypeDetails));
      mutate.apply(m => m
        .updateKey(newProperty.value)
