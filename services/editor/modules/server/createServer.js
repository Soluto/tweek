...
export default function createServer(getApp) {
  const server = express();
  const webpackStats = getWebpackStats();
  server.disable('x-powered-by');
  server._listen = server.listen;
  server.listen = () => {
    throw new Error('[tweek-editor]', 'Do not call `server.listen()`, use `server.start()`');
  };
...
function getContent(req, appElement) {
  return SERVER_RENDERING ? renderToString(appElement) : '';
}
...
function getAssetPaths(stats, regex) {
  return Object.keys(stats.assetsByChunkName).reduce((assets, key) => {
    ...
    return assets.concat(
      chunkArray.filter(asset => regex.test(asset)).map(asset => stats.publicPath + asset),
    );
  }, []);
}
...
function getJavaScriptTags(stats) {
  return getAssetPaths(stats, /\.js$/).map(src => <script key={src} src={src} />);
}
