...
import { DEV_PORT, DEV_HOST, PUBLIC_PATH, SERVER_RENDERING, NODE_ENV } from './webpack.shared.config'
...
function getCSSLoader() {
  const loader = { test: SHARED.CSS_REGEX, exclude: /node_modules/ };
...
function getEntry() {
  const entry = {
    app:  path.join(SHARED.APP_PATH, 'modules/client.js'),
    _vendor: [ 'react', 'react-dom', 'react-router' ]
  };
  if (!PROD) {
    if (HOT) {
