...
if [[ "$(git tag | grep -c tweek-api-$TWEEK_API_VERSION)" == "0" ]]; then
    ...
    echo "TWEEK_API_VERSION=$TWEEK_API_VERSION" >> $GITHUB_ENV
fi
...
if [[ "$(git tag | grep -c tweek-publishing-$TWEEK_PUBLISHING_VERSION)" == "0" ]]; then
    ...
    echo "TWEEK_PUBLISHING_VERSION=$TWEEK_PUBLISHING_VERSION" >> $GITHUB_ENV
fi
...
if [[ "$(git tag | grep -c tweek-authoring-$TWEEK_AUTHORING_VERSION)" == "0" ]]; then
    ...
    echo "TWEEK_AUTHORING_VERSION=$TWEEK_AUTHORING_VERSION" >> $GITHUB_ENV
fi
...
if [[ "$(git tag | grep -c tweek-editor-$TWEEK_EDITOR_VERSION)" == "0" ]]; then
    ...
    echo "TWEEK_EDITOR_VERSION=$TWEEK_EDITOR_VERSION" >> $GITHUB_ENV
fi
...
if [[ "$(git tag | grep -c tweek-gateway-$TWEEK_GATEWAY_VERSION)" == "0" ]]; then
    ...
    echo "TWEEK_GATEWAY_VERSION=$TWEEK_GATEWAY_VERSION" >> $GITHUB_ENV
fi
